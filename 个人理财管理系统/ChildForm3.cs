...
namespace 个人理财管理系统
{
    public partial class ChildForm3 : Form
    {
        ...
        private void DataGridInit()
        {
            ...
            {
                Category category = Tools.Instance.FindCategoryById(items[i].CategoryId, categories);
        ...
        private void AddButton()
        {
            ...
            if (result == DialogResult.OK)
            {
                ...
                Category category = Tools.Instance.FindCategoryByName(comboBox1.SelectedItem.ToString(), categories);
                string remark = textBox2.Text;
                if (Tools.Instance.FindItemByName(itemname, items) != null)
                {
        ...
        private void ModifyButton()
        {
            ...
            string remark = textBox2.Text;
            UpdateItemWithTransaction(connectionString, itemId, newname, categoryId, remark);
            items = Tools.Instance.GetItemTable();
            DataGridInit();
        }
        private static int UpdateItemWithTransaction(string connectionString, int itemId, string newName, int? categoryId, string remark)
        {
            int successRow = 0;
                ...
                try
                {
                    ...
                    using (SqlTransaction transaction = connection.BeginTransaction())
                    {
                        using (SqlCommand command = new SqlCommand(updateQuery, connection, transaction))
                        {
                            ...
                            command.Parameters.AddWithValue("@itemId", categoryId); // 处理可空值
